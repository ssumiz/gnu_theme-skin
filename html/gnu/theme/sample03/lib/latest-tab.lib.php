...
// 최신글 추출
// $cache_time 캐시 갱신시간
function latest_tab($skin_dir = '', $bo_table, $rows = 10, $subject_len = 40, $cache_time = 1, $options = '')
{
  global $g5;
  ...
  if (G5_USE_CACHE) {
    ...
    $cache_bo_categories = isset($caches['bo_categories']) ? $caches['bo_categories'] : array();
  ...
  if ($caches === false) {
    ...
    foreach ($bo_categories as $ck => $cv) {
      $sql = " select * from {$tmp_write_table} where wr_is_comment = 0 and ca_name='" . sql_real_escape_string($cv) . "' order by wr_num limit 0, {$rows} ";
      ...
      for ($i = 0; $row = sql_fetch_array($result); $i++) {
        ...
        $list[$ck][$i] = get_list($row, $board, $latest_skin_url, $subject_len);

        $list[$ck][$i]['first_file_thumb'] = (isset($row['wr_file']) && $row['wr_file']) ? get_board_file_db($bo_table, $row['wr_id'], 'bf_file, bf_content', "and bf_type between '1' and '3'", true) : array('bf_file' => '', 'bf_content' => '');
        $list[$ck][$i]['bo_table'] = $bo_table;
        // 썸네일 추가
        if ($options && is_string($options)) {
          ...
          if ($thumb['src']) {
            ...
            $list[$ck][$i]['img_thumbnail'] = '<a href="' . $list[$ck][$i]['href'] . '" class="lt_img">' . $img_content . '</a>';
            // } else {
    ...
    $bo_categories = $cache_bo_categories;
  }

  ob_start();
  include $latest_skin_path . '/latest_tab.skin.php';
